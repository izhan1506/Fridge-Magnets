...
// Served from this origin. Upstream points both of these at a third-party CDN
// and pulls a 1.4MB NASA photograph, which makes a marketing page depend on
// that CDN at runtime. These are the same NASA maps, self-hosted, downscaled to
// 1024x512 (the globe never renders wider than ~380px) and re-encoded to WebP:
// 1428KB + 378KB of jpg/png becomes 81KB + 18KB.
const DEFAULT_EARTH_TEXTURE = "/globe/earth-blue-marble.webp";
const DEFAULT_BUMP_TEXTURE = "/globe/earth-topology.webp";
...
function Marker({
  ...
  const surfacePosition = useMemo(() => {
    return latLngToVector3(marker.lat, marker.lng, radius * MARKER_SURFACE);
  }, [marker.lat, marker.lng, radius]);

  // Top of the line (where the image is) - positioned further out to prevent going inside globe
  const topPosition = useMemo(() => {
    return latLngToVector3(marker.lat, marker.lng, radius * MARKER_TOP);
  }, [marker.lat, marker.lng, radius]);
...
function RotatingGlobe({
  ...
  return (
    // `initialRotation` is declared in Globe3DConfig upstream but never applied
    // anywhere, so the globe always started on the mid-Pacific. Honoured here:
    // markers are inside this group, so it sets which longitude faces the
    // camera on arrival.
...
function Atmosphere({ radius, color, intensity, blur }: AtmosphereProps) {
  ...
  return (
    // 1.08, not upstream's 1.12. The shell's fresnel peaks at ITS silhouette,
    // so the wider it is the further the halo floats off the planet; at 1.12 it
    // reads as a detached hoop rather than atmosphere.
    ...
    <div className={cn("relative h-[500px] w-full", className)}>
      <Canvas
        ...
        camera={{
          ...
          near: 0.1,
          far: 1000,
          position: [0, 0, mergedConfig.radius * 3.0],
        }}
        style={{
